...
export default function App() {
  ...
  const handleDownloadPng = () => {
    ...
    img.onload = () => {
      ...
      const renderWidth = parseFloat(svgElement.getAttribute('width') || '1000');
      const renderHeight = parseFloat(svgElement.getAttribute('height') || '1000');
      
      const scale = 2; // Exporting at 2x resolution
      ...
      <header className="h-14 border-b border-slate-200 bg-white flex items-center justify-between px-6 shrink-0 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-1.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-md transition-all shadow-sm border border-slate-200"
          >
            <Download size={14} />
            SVG保存
          </button>
        ...
        <div className="flex-1 overflow-hidden relative border-t border-slate-200 lg:border-t-0 bg-[#fbfcfd]">
          ...
          <div className="absolute bottom-6 left-6 p-4 bg-white/60 backdrop-blur-md border border-slate-200 rounded-xl shadow-lg pointer-events-none hidden md:block">
            ...
            <div className="flex gap-4">
              ...
              <LegendItem label="準備" shape="preparation" />
...
function LegendItem({ label, shape }: { label: string; shape: string }) {
  const renderIcon = () => {
    ...
    switch (shape) {
      ...
      case 'preparation': return <path d="M 5 0 L 25 0 L 30 7.5 L 25 15 L 5 15 L 0 7.5 Z" className={common} />;
